...
function start(callback) {
  const server = http.createServer((req, res) => {
    ...
    if (method != "PUT") {
      ...
      res.end(distribution.util.serialize(
        Error("This endpoint only accepts PUT.")
      ));
      return;
    }
    ...
    */
    ...
    console.log(url);
    console.log(paths);
    const [gid, route, rmethod] = paths.slice(1);
    ...
    */
    ...
    req.on('end', () => {
      ...
      */
      ...
      const args = distribution.util.deserialize(full_body);
      ...
      if (!distribution[gid]) {
        ...
        res.end(distribution.util.serialize([Error("Server: Group not found."), null]));
        return;
      }
      distribution[gid].routes.get(route, (e1, s1) => {
        if (e1) {
          res.writeHead(404, {'Content-Type': 'application/json'});
          res.end(distribution.util.serialize([e1, s1]));
          return;
        } else if (!(rmethod in s1)) {
          res.writeHead(404, {'Content-Type': 'application/json'});
          res.end(distribution.util.serialize([Error("Method not found on service."), null]));
          return;
        } else {
          s1[rmethod](...args, (e2, v2) => {
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(distribution.util.serialize([e2, v2]))
          });
        }
      })
