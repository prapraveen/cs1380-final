...
test('(5 pts) (scenario) use mem.reconf', (done) => {
  ...
  */
  ...
  const keysAndItems = [
    ...
    {key: 'b', item: {first: 'Bob', last: 'b'}},
    {key: 'c', item: {first: "Charlie", last: 'c'}},
    {key: 'd', item: {first: "Delta", last: 'd'}},
    {key: 'e', item: {first: "Eve", last: 'e'}}
  ];
  ...
  distribution.local.groups.put(config, mygroupGroup, (e, v) => {
    ...
    const checkPlacement = (e, v) => {
      ...
      const remote1 = {node: n1, service: 'mem', method: 'get'};
      const remote2 = {node: n2, service: 'mem', method: 'get'};
      const remote3 = {node: n3, service: 'mem', method: 'get'};
      distribution.local.comm.send(messages[0], remote2, (e, v) => {
        ...
        distribution.local.comm.send(messages[1], remote1, (e, v) => {
          try {
            expect(e).toBeFalsy();
          ...
          distribution.local.comm.send(messages[2], remote1, (e, v) => {
            try {
              expect(e).toBeFalsy();
            ...
            distribution.local.comm.send(messages[3], remote1, (e, v) => {
              try {
                expect(e).toBeFalsy();
                expect(v).toEqual(keysAndItems[3].item);
              } catch (error) {
                done(error);
                return;
              }
              distribution.local.comm.send(messages[4], remote2, (e, v) => {
                try {
                  expect(e).toBeFalsy();
                  expect(v).toEqual(keysAndItems[4].item);
                } catch (error) {
                  done(error);
                  return;
                }
  ...
  const runSolution = () => {
    ...
    const processNode = (node, dataToProcess, callback) => {
      ...
      for (let i = 0; i < entries.length; i++) {
        new_obj[entries[i][0]] = entries[i][1];
      }
      distribution.shuffleGroup.store.append(entry[1], entry[0], () => {processNode(node, new_obj, callback)});
    };
