...
*/
  ...
  const mapper = (key, value) => {
      const res = [];
      const counts = {};
      // const total_terms = new Set(value.split(" ")).size;
      const total_terms = value.split(" ").length;
      for (const term of value.split(" ")) {
          if (!counts[term]) {
              counts[term] = 0;
          }
          counts[term] += 1 / total_terms;
      }

      for (const term of Object.keys(counts)) {
          res.push({[term]: {[key]: counts[term]}});
      }
      return res;
  };
  ...
  const reducer = (key, values) => {
    ...
    for (const value of values) {
        const doc = Object.keys(value)[0];
        const tf = Object.values(value)[0];
        const tf_idf = tf * idf;
        final_val[doc] = Math.round(tf_idf * 100) / 100;
    }
...
test('(10 pts) (scenario) all.mr:crawl', (done) => {
    const dataset = [
        {1: "https://cs.brown.edu/courses/csci1380/sandbox/1/"},
        {2: "https://cs.brown.edu/courses/csci1380/sandbox/2/"},
        {3: "https://cs.brown.edu/courses/csci1380/sandbox/3/"},
        {4: "https://cs.brown.edu/courses/csci1380/sandbox/4/"},
    ];
    ...
    const reducer = (key, values) => {
        // there should only be one value per key anyway
        const value = values.reduce((a, b) => b, '');
        return {[key]: value};
    }
...
test('(10 pts) (scenario) all.mr:strmatch', (done) => {
    // split by lines, then try to match on each line
    // emit per match per line
    //
    const dataset = [
        {'1': `hello world
    ...
    const expected = ['abcdefg', 'hello world', 'distributed', 'systems'];
    ...
    const doMapReduce = () => {
        distribution.str.store.get(null, (e, v) => {
            ...
            distribution.str.store.get("1", (e, v) => {
            ...
            distribution.dlib.groups.put(dlibConfig, dlibGroup, (e, v) => {
              ...
              distribution.local.groups.put(tfidfConfig, tfidfGroup, (e, v) => {
                distribution.tfidf.groups.put(tfidfConfig, tfidfGroup, (e, v) => {
                    const crawlConfig = {gid: 'crawlGroup'};
                    distribution.local.groups.put(crawlConfig, crawlGroup, (e, v) => {
                        distribution.crawlGroup.groups.put(crawlConfig, crawlGroup, (e, v) => {
                            const strConfig = {gid: 'str'};
                            distribution.local.groups.put(strConfig, strmatchGroup, (e, v) => {
                                distribution.str.groups.put(strConfig, strmatchGroup, (e, v) => {
                                    done();
                                });
                            });
                        });
